...
export class UserService {
  ...
  async getsixmois(): Promise<User[]> {
    try {
      ...
      const user = await this.userRepository.find({
        ...
        $expr: { $lt: [{ $toDate: '$updatedAt' }, sixmois] },
      });
      return user;
    } catch (err) {
      return [];
  ...
  async getPaginatorsUsers(page: number, limit: number) {
    try {
  ...
  async Desactivate() {
    try {
      ...
      const user = await this.userRepository.updateMany(
        {
          $expr: { $lte: [{ $toDate: '$updatedAt' }, sixmois] },
        },
        { $set: { status: 'not done' } },
      );
      return user;
  ...
  async updateRole(domain: string, myrole: string) {
    try {
      const user = await this.userRepository.updateMany(
        {
          email: { $regex: `@${domain}$` },
